...
// gets the minimum of a population of data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
int Get_Minimum(vector<int>& y_data, int ndv)
{
  int min = 10000;
  for (int i =0; i < int(y_data.size()); i++)
  {
...
// gets the maximum of a population of data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
int Get_Maximum(vector<int>& y_data, int ndv)
{
  float max = -9999999999;
  for (int i =0; i < int(y_data.size()); i++)
  {
...

{
  ...
  int NBins = int( (upper_limit - lower_limit)/bin_width )+1;
  cout << "Binning your data. Upper limit: " << upper_limit << " Lower limit: " << lower_limit << " NBins: " << NBins << endl;
...
///
///===========================================================================
void createGeoJSONPolygon(const vector<double>& latitudes, const vector<double>& longitudes, const string& filename) 
{
    if (latitudes.size() 
    != longitudes.size() || latitudes.empty()) {
        cerr << "Error: Latitude and longitude vectors must be of the same size and not empty." << std::endl;
        return;
    }

    ofstream geojsonFile(filename);
    ...
    geojsonFile << "        \"type\": \"Polygon\",\n";
    geojsonFile << "        \"coordinates\": [\n";
    geojsonFile << "          [\n";

    for (size_t i = 0; i < latitudes.size(); ++i) {
        geojsonFile << "            [" << longitudes[i] << ", " << latitudes[i] << "],\n" ;
    }

    // Close the polygon by repeating the first point
    geojsonFile << "            [" << longitudes[0] << ", " << latitudes[0] << "]\n";
    ...
    geojsonFile.close();
    cout << "GeoJSON polygon file created successfully: " << filename << std::endl;
}

void spiral_search(vector<int>& row_increment, vector<int>& col_increment)
{
  vector<int> c_increment;
  vector<int> r_increment; 

  c_increment.push_back(1);  r_increment.push_back(0);
  c_increment.push_back(0);  r_increment.push_back(-1);
  c_increment.push_back(-1);  r_increment.push_back(0);
  c_increment.push_back(-1);  r_increment.push_back(0); 
  c_increment.push_back(0);  r_increment.push_back(1); 
  c_increment.push_back(0);  r_increment.push_back(1); 
  ...
  row_increment = r_increment;
  col_increment = c_increment;
}
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--==
// This function fixes the path, mainly to add a slash to the end of the path
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--==
